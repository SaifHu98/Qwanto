...
def build_report(
    ...
    return {
        ...
        "format_status": [
            ...
            {"format": "Q4_0", "status": "validated conversion and measured native qwnrun evidence" if any(row["qwn_quantization"] == "Q4_0" and row["evidence_class"] == "MEASURED" for row in native_rows) else "implemented and container-validated; no current native inference row", "evidence_class": "MEASURED" if any(row["qwn_quantization"] == "Q4_0" and row["evidence_class"] == "MEASURED" for row in native_rows) else "UNAVAILABLE"},
            {"format": "HyperVSQ-2", "status": "validated conversion and measured native qwnrun evidence" if any(row["qwn_quantization"] == "HyperVSQ-2" and row["evidence_class"] == "MEASURED" for row in native_rows) else "validated conversion; native measurement unavailable", "evidence_class": "MEASURED" if any(row["qwn_quantization"] == "HyperVSQ-2" and row["evidence_class"] == "MEASURED" for row in native_rows) else "UNAVAILABLE"},
            ...
            {"format": "Q8_0", "status": "implemented container dtype; no current native inference row", "evidence_class": "UNAVAILABLE"},
            {"format": "BF16", "status": "implemented container dtype; no current native inference row", "evidence_class": "UNAVAILABLE"},
            {"format": "VSQ", "status": "implemented container dtype; local reference matrix only", "evidence_class": "EXPERIMENTAL"},
            {"format": "VSQ_ULTRA", "status": "implemented container dtype; local reference matrix only", "evidence_class": "EXPERIMENTAL"},
            {"format": "HYPER_VSQ", "status": "implemented container dtype; local reference matrix only (distinct from HyperVSQ-2)", "evidence_class": "EXPERIMENTAL"},
