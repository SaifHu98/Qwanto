...
| `--queue-timeout` | `$QWANTO_QUEUE_TIMEOUT` or `300` | Seconds a request may wait. |
| `--kv-slots` | `$QWANTO_KV_SLOTS` or `1` | Independent KV conversation slots (→ `KV_SLOTS`). |
...
Run directly (or via `qwanto serve`). OpenAI-compatible `/v1/chat/completions`.

| Flag | Default | Meaning |
|---|---|---|
| `--model` | `$QWANTO_MODEL` (required if unset) | Model snapshot directory. |
...
| `--queue-timeout` | `$QWANTO_QUEUE_TIMEOUT` or `300` | Request queue timeout (s). |
| `--kv-slots` | `$QWANTO_KV_SLOTS` or `1` | KV conversation slots. |

Tool calling (`tools` in the request) is supported; the opt-in `QWANTO_TOOL_SALVAGE=1` env var recovers malformed int4 tool calls. Server-relevant env vars: `QWANTO_METAL`, `PIPE`, `DIRECT`, `QWANTO_NO_OMP_TUNE`, `RAM_GB`, `CTX`, `KVSAVE` (all from [ENVIRONMENT.md](ENVIRONMENT.md)) apply because the server launches the same `glm` engine.
...
- For knobs with **no** flag (`QWANTO_METAL`, `PIPE`, `DIRECT`, `QWANTO_NO_OMP_TUNE`, `MLOCK`, `CAP_RAISE`, `KVSAVE`, `SEED`, `NUCLEUS`, …), export the environment variable.
