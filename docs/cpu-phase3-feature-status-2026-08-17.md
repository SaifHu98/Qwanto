...
| Delayed VNNI reduction | `VALIDATED_AND_ENABLED` locally | Production default; `QWN_HYPERVSQ2_DISABLE_DELAYED_REDUCTION=1` is an explicit developer ablation override. Clean-commit 64-token median `18.985890` vs `17.764969` baseline; 128-token median `18.945001` vs `17.737912`. Counter is nonzero and streamed output agrees exactly. |
| HyperVSQ-2 multi-row blocking | `VALIDATED_NOT_BENEFICIAL` | Exact 74-byte candidates pass differential tests, but clean-commit row-2 `15.680256` and row-4 `14.571689` tok/s are slower than the same-build delayed path; row-block counter is zero in production. |
...
| CPU affinity | `VALIDATED_NOT_BENEFICIAL` | OS-default wins clean-commit release-quality 64-token (`18.890454`) and 128-token (`18.364016`) controls over close/spread. No affinity policy is enabled by default. |
...
|---|---:|---:|---:|---:|---|
| 64 generated tokens | `18.985890` tok/s | `18.814534` | `3401.626 ms` | `18.919199` tok/s | proven |
| 128 generated tokens | `18.945001` tok/s | `18.726433` | `6835.258 ms` | `19.176576` tok/s | proven |

The matching same-binary delayed-disabled controls were `17.764969` tok/s and
`17.737912` tok/s respectively. These values are local pending-hosted evidence,
not README claims.

## Roofline correction

The final report uses the selected 8-worker aggregate read proxy of
`34,421,311,165.34 bytes/s` and executed logical total bytes of
`481,038,007.52 bytes/token`:

```text
34,421,311,165.34 / 481,038,007.52 = 71.556323 tok/s
```
...
- Source commit for clean evidence: `9a686913d33e9621c95955b53e8b9e980cb01456`; worktree dirty: `false`.
...
- Final delayed 64/128: `benchmarks/evidence/windows/2026-08-17/phaseA-clean-9a68691/final-delayed-8t-64.json`, `final-delayed-8t-128.json`
- Same-build baseline 64/128: `phaseA-clean-9a68691/final-baseline-8t-64.json`, `final-baseline-8t-128.json`
- Row candidates: `phaseA-clean-9a68691/final-row2-8t-64.json`, `final-row4-8t-64.json`
- Affinity matrices: `phaseA-clean-9a68691/affinity-final/`, `affinity-final-128/`
- Autotune: `phaseA-clean-9a68691/autotune-final.json` and `autotune-final-cache.json`
- Ablation: `benchmarks/evidence/windows/2026-08-17/phaseA-clean-9a68691/phaseA-ablation-final.json`, `docs/cpu-phaseA-ablation-2026-08-17.md`
