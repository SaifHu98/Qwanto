  ...
  return (
    <div className="brain-page">
      <div className="brain-head">
        <div className="section-title"><BrainCircuit className="size-4" /> Expert Cortex — {data ? `${data.rows} layers × ${data.cols} experts` : connected ? "loading..." : "connect to engine to visualize"}</div>
      ...
      <div className="brain-canvas-wrap" ref={wrapRef}>
        ...
        {!connected && <p className="runtime-unavailable">Connect to the engine to see the cortex. The expert grid shows which experts are loaded in VRAM (green), RAM (blue), or on disk (dark).</p>}
        {connected && !data && <p className="runtime-unavailable">
          <div className="brain-unavailable-box">
            <BrainCircuit className="size-8 text-muted-foreground mb-2" />
            <div className="font-bold text-sm mb-1">Expert Cortex not available</div>
            <div className="text-xs text-muted-foreground">This feature is only available for native MoE models (GLM-5.2, Qwen-MoE, DeepSeek-MoE).</div>
            <div className="text-xs text-muted-foreground mt-1">You are using a GGUF model via llama.cpp backend.</div>
          </div>
        </p>}
      </div>
      {tip && data && (() => {
