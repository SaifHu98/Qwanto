...
const qwantoApi = {
  stream: (options: {
    ...
    gpuDevice?: number;
    forceCpu?: boolean;
  }) => ipcRenderer.invoke("qwanto:stream", options),
  cancel: () => ipcRenderer.invoke("qwanto:cancel"),
  onToken: (sessionId: string, callback: (data: { token: string }) => void) => {
    const listener = (_event: IpcRendererEvent, data: any) => callback(data);
    ipcRenderer.on(`qwanto:token:${sessionId}`, listener);
    return () => ipcRenderer.removeListener(`qwanto:token:${sessionId}`, listener);
  },
  onTelemetry: (sessionId: string, callback: (data: { tokPerSec: number; ttftMs: number; totalTokens: number }) => void) => {
    const listener = (_event: IpcRendererEvent, data: any) => callback(data);
    ipcRenderer.on(`qwanto:telemetry:${sessionId}`, listener);
    return () => ipcRenderer.removeListener(`qwanto:telemetry:${sessionId}`, listener);
  },
  onDone: (sessionId: string, callback: (data: { totalTokens: number; fullText: string }) => void) => {
    const listener = (_event: IpcRendererEvent, data: any) => callback(data);
    ipcRenderer.on(`qwanto:done:${sessionId}`, listener);
    return () => ipcRenderer.removeListener(`qwanto:done:${sessionId}`, listener);
  },
  onError: (sessionId: string, callback: (data: { error: string }) => void) => {
    const listener = (_event: IpcRendererEvent, data: any) => callback(data);
    ipcRenderer.on(`qwanto:error:${sessionId}`, listener);
    return () => ipcRenderer.removeListener(`qwanto:error:${sessionId}`, listener);
  },
  models: {
    list: () => ipcRenderer.invoke("qwanto:models:list"),
    inspect: (filePath: string) => ipcRenderer.invoke("qwanto:models:inspect", filePath),
    addDir: (dirPath: string) => ipcRenderer.invoke("qwanto:models:addDir", dirPath),
  },
  telemetry: {
    get: () => ipcRenderer.invoke("qwanto:telemetry:get"),
  },
  permission: {
    setMode: (mode: "plan" | "agent") => ipcRenderer.invoke("qwanto:permission:setMode", mode),
    getMode: () => ipcRenderer.invoke("qwanto:permission:getMode"),
    setWorkspace: (root: string) => ipcRenderer.invoke("qwanto:permission:setWorkspace", root),
    audit: () => ipcRenderer.invoke("qwanto:permission:audit"),
  },
  tools: {
    readFile: (filePath: string, startLine?: number, endLine?: number) =>
      ipcRenderer.invoke("qwanto:tools:readFile", filePath, startLine, endLine),
    writeFile: (filePath: string, content: string) =>
      ipcRenderer.invoke("qwanto:tools:writeFile", filePath, content),
    editFile: (filePath: string, target: string, replacement: string) =>
      ipcRenderer.invoke("qwanto:tools:editFile", filePath, target, replacement),
    listDirectory: (dirPath?: string) =>
      ipcRenderer.invoke("qwanto:tools:listDirectory", dirPath),
    bash: (command: string, customCwd?: string) =>
      ipcRenderer.invoke("qwanto:tools:bash", command, customCwd),
  },
  sessions: {
    list: () => ipcRenderer.invoke("qwanto:sessions:list"),
    get: (sessionId: string) => ipcRenderer.invoke("qwanto:sessions:get", sessionId),
    save: (session: any) => ipcRenderer.invoke("qwanto:sessions:save", session),
    delete: (sessionId: string) => ipcRenderer.invoke("qwanto:sessions:delete", sessionId),
  },
  apiServer: {
    start: (port?: number) => ipcRenderer.invoke("qwanto:apiServer:start", port),
    stop: () => ipcRenderer.invoke("qwanto:apiServer:stop"),
    status: () => ipcRenderer.invoke("qwanto:apiServer:status"),
  },
};

contextBridge.exposeInMainWorld("clientCore", clientCoreApi);
contextBridge.exposeInMainWorld("qwanto", qwantoApi);
