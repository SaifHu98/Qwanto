...
typedef int            (*fn_qwn_matmul)(ColiCudaTensor **tensor,float *y,const float *x,
...
static int coli_cuda_load(void){
    ...
    /* Optional for older installed DLLs: native .qwn falls back to CPU while
     * all pre-existing CUDA paths remain available. */
    g_cuda.qwn_matmul=(fn_qwn_matmul)GetProcAddress(g_cuda.dll,"coli_cuda_qwn_matmul");
...
int coli_cuda_qwn_matmul(ColiCudaTensor **tensor,float *y,const float *x,
                         const void *blocks,int S,int I,int O,int device){
    if(!g_cuda.available||!g_cuda.qwn_matmul)return 0;
    return g_cuda.qwn_matmul(tensor,y,x,blocks,S,I,O,device);
}
