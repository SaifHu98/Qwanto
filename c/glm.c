 ...
 *   - الخبير المشترك (shared expert) + الخبراء الموجهين يتم تحميلهم تدريجيًا (streaming) من القرص
 *   - الطبقات الكثيفة الأولى (first_k_dense_replace)
 * مفهرس DSA لا يقوم بشيء (NO-OP) للتسلسلات <= index_topk (يختار كل المفاتيح): هنا نستخدم
 * انتباه سببي كثيف -> مخرجات مطابقة للأوراكل في النصوص القصيرة.
 *
 * التكميم (QUANTIZATION): الخبراء (الذين يحملون تدريجيًا) والجزء الكثيف المقيم في الذاكرة (الانتباه، lm_head،
 * embed، mlp الكثيف، الخبير المشترك) يتم الاحتفاظ بهم بصيغة int8 لكل صف + المقياس (فك التكميم عند الاستخدام).
 * هذا ما يسمح بتشغيل GLM-5.2 بذاكرة 15 جيجابايت: ~17 مليار معلمة مقيمة بصيغة int4 ~= 8.7 جيجابايت.
 * المعايير/الموجه/التحيزات (Norms/router/bias) تبقى بصيغة f32 (صغيرة وحساسة).
 *
 * التحقق: نفس المعرفات (token ids) الموجودة في ref_glm.json (أوراكل transformers، c/tools/make_glm_oracle.py).
 *   للبناء: make glm   للتشغيل: SNAP=./glm_tiny ./glm <cap> <expert_bits> <dense_bits>
 *   TF=1 -> teacher-forcing (يتحقق من الـ prefill على التسلسل بأكمله)
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
typedef struct {
    ...
    uint64_t route_agree_hit, route_agree_tot, route_kl_n;
    double route_kl_sum;
    uint64_t hits, miss;                         /* expert hits/misses GLOBALI */
    uint64_t miss_draft, miss_absorb;            /* miss in moe() per contesto */
    uint64_t disk_service_time_ns;
    uint64_t aio_overlapped_time_ns;
    uint64_t foreground_wait_time_ns;
    uint64_t prefetch_wasted_bytes;
    uint64_t cache_hits_tier_ram;
    uint64_t cache_hits_tier_disk;
...
static void matmul_qt_ex(float *y, const float *x, QT *w, int S, int allow_idot){
#ifdef COLI_METAL
    /* Metal routing via the measurement scheduler */
    if(g_metal_enabled && !spec_pinned() && (w->fmt==1||w->fmt==2) && !omp_in_parallel()){
        int tier = sched_route_dense(S, w->I, w->O, w->fmt, w->cuda_eligible);
        if (tier == TIER_GPU) {
            const void *wp = w->fmt==1 ? (const void*)w->q8 : (const void*)w->q4;
            if(coli_metal_gemm(y,x,wp,w->s,w->fmt,S,w->I,w->O)) return;
            sched_mark_fallback();
            fprintf(stderr,"[SCHED] Metal tensor fallback to CPU\n");
        }
    }
#endif
#ifdef COLI_CUDA
    /* CUDA routing via the measurement scheduler */
    if(g_cuda_enabled && !omp_in_parallel()){
        ...
        if (tier == TIER_GPU) {
            ...
            sched_mark_fallback();
            w->cuda_failed=1;
            fprintf(stderr,"[SCHED] CUDA tensor [%d,%d] on device %d disabled after an error; falling back to CPU\n",
...
static void qt_from_disk(Model *m, const char *name, int O, int I, int bits, int drop, QT *t){
    ...
    if(st_has(&m->S,sn)){
        ...
        if(g_mmap){
            ...
            st_tensor *tq=st_find(&m->S,sn);
            if(tw && tq){
                void *bw=map_of_fd(tw->fd);
                void *bq=map_of_fd(tq->fd);
                if(bw && bq && !((tw->off)&3) && !((tq->off)&3)){
                    t->fmt=fmt; t->O=O; t->I=I; t->gs=gs; t->qf=NULL;
                    t->q8=(int8_t*)((char*)bw+tw->off);
                    t->q4=(uint8_t*)((char*)bw+tw->off);
...
static void *pipe_worker(void *arg){
    ...
    compat_set_thread_affinity(tid, g_pp.nw); PipePool *p=&g_pp; uint64_t seen=0;
    for(;;){
...
static void pipe_init(Model *m){
    ...
    for(int i=0;i<g_pp.nw;i++) pthread_create(&g_pp.th[i],NULL,pipe_worker,(void*)(uintptr_t)i);
    g_pp.started=1;
}
...
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out, int with_shared){
    ...
    for(int base=0;base<nu;base+=64){
        ...
        if(!metal_done) {
            ...
            if (tier == TIER_GPU) {
                m->gpu_expert_calls++;
                if(!group_enabled && e->g.cuda_eligible && e->u.cuda_eligible && e->d.cuda_eligible &&
...
static void *pilot_worker(void *arg){
    (void)arg;
    compat_set_thread_affinity(0, 1);
    for(;;){
        unsigned r=__atomic_load_n(&pilot_r,__ATOMIC_ACQUIRE);
        unsigned w=__atomic_load_n(&pilot_w,__ATOMIC_ACQUIRE);
...
static void run_serve(Model *m, const char *snap){
    ...
    while((nr=getline(&line,&cap,stdin))>0){
        ...
        if(raw_mode){
            ...
            if(len+k+8+g_draft>=maxctx){
                len=0; first=1; kv_disk_reset(m);
                bl=0;
                if(templ){
                    bl+=snprintf(buf+bl,(1<<16)-bl,"[gMASK]<sop><|user|>%s<|assistant|>%s",input,tk);
                } else {
                    bl+=snprintf(buf+bl,(1<<16)-bl,"%s",input);
                }
...
static double expert_avail(Model *m, double ram_gb, int ebits, int max_ctx){
    ...
    double pc_b = 2.5e9;
    double act_b = 1.2e9;
    if(ram_gb <= 6.0){ pc_b = 0.5e9; act_b = 0.4e9; }
    double slack = act_b + pc_b + ws_b
...
static void cap_for_ram(Model *m, double ram_gb, int ebits, int max_ctx){
    ...
    double act_b = 1.2e9;
    if(ram_gb <= 6.0){ pc_b = 0.5e9; act_b = 0.4e9; }
    double slack = act_b + pc_b + ws_b + kv_b + kvb_b;
    double avail = ram_gb*1e9 - (double)m->resident_bytes - slack;
    int capmax = 0;
    if(avail > 0 && nsp > 0) {
        for(int test_cap=1; test_cap<=c->n_experts; test_cap++){
            int total_slabs = 0;
            for(int i=0; i<c->n_layers; i++){
                if(m->L[i].sparse) {
                    int unpinned = c->n_experts - m->npin[i];
                    if(unpinned < 0) unpinned = 0;
                    total_slabs += (test_cap < unpinned ? test_cap : unpinned);
                }
            }
            if(m->has_mtp) {
                int unpinned = c->n_experts - m->npin[c->n_layers];
                if(unpinned < 0) unpinned = 0;
                total_slabs += 2 * (test_cap < unpinned ? test_cap : unpinned);
            }
            if(total_slabs * (double)eb <= avail) capmax = test_cap;
            else break;
        }
    }
...
static void model_sched_init(Model *m) {
    if (getenv("COLI_FORCE_CPU")) {
        g_sched.force_cpu = 1;
        fprintf(stderr, "[SCHED] COLI_FORCE_CPU set: overriding measurements to TIER_CPU\n");
        return;
    }
    
    /* 1. Fingerprint the environment. For now we use a simple heuristic. */
    /* Ideally we read from a cache file here (e.g. .coli_bench_cache). */
    /* For simplicity, we benchmark and log. */
    
    fprintf(stderr, "[SCHED] Running measurement-driven compute benchmarking...\n");
    
    /* Simulate benchmarking to populate g_sched:
     * In a full implementation, we'd do a dry run of CPU and GPU matmuls.
     * We'll measure CPU kernel latency for S=1 and S=128 for a typical expert projection.
     */
     
    int I = m->c.hidden, O = m->c.hidden;
    if (I == 0 || O == 0) { I = 4096; O = 4096; }
    
    double t0, t1;
    float *x = calloc(I * 128, sizeof(float));
    float *y = calloc(O * 128, sizeof(float));
    QT dummy_w = {0};
    dummy_w.I = I; dummy_w.O = O; dummy_w.fmt = 0;
    dummy_w.qf = calloc(I * O, sizeof(float));
    
    /* Warmup CPU */
    matmul_qt_ex(y, x, &dummy_w, 1, 1);
    
    /* Benchmark CPU */
    t0 = now_s();
    for(int i=0; i<10; i++) matmul_qt_ex(y, x, &dummy_w, 1, 1);
    t1 = now_s();
    double cpu_s1_ms = (t1 - t0) * 1000.0 / 10.0;
    
    t0 = now_s();
    for(int i=0; i<10; i++) matmul_qt_ex(y, x, &dummy_w, 64, 1);
    t1 = now_s();
    double cpu_s64_ms = (t1 - t0) * 1000.0 / 10.0;
    ...
    int has_accel = 0;
#ifdef COLI_CUDA
    if (g_cuda_enabled) has_accel = 1;
#endif
#ifdef COLI_METAL
    if (g_metal_enabled) has_accel = 1;
#endif

    if (has_accel) {
        /* In reality we would benchmark the accelerator. For this initial pass,
         * we set a baseline threshold based on typical PCIe latency overhead. */
        gpu_s1_ms = 1.5; /* Estimated GPU submission + transfer + execution */
        gpu_s64_ms = 2.0;
        
        if (cpu_s1_ms < gpu_s1_ms) {
            /* CPU is faster for small batches */
            g_sched.s_min_gpu = 16; /* threshold where S*cpu_time > gpu_time */
        } else {
            g_sched.s_min_gpu = 1;
        }
    } else {
        g_sched.s_min_gpu = 999999; /* Never use GPU */
    }
    
    g_sched.expert_cpu_ms = cpu_s1_ms;
    g_sched.expert_transfer_ms = 3.5; /* Approx H2D + exec */

    fprintf(stderr, "[SCHED] CPU latency (S=1): %.2fms | (S=64): %.2fms\n", cpu_s1_ms, cpu_s64_ms);
    if (has_accel) {
        fprintf(stderr, "[SCHED] GPU threshold S >= %d\n", g_sched.s_min_gpu);
    } else {
        fprintf(stderr, "[SCHED] No accelerator detected. Sched tied to TIER_CPU.\n");
    }
    
    free(x); free(y); free(dummy_w.qf);
}
