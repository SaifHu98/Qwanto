...
static void test_matrix_mapping(void) {
    ...
    EXPECT_NEAR(qa.speedup_target, 0.0f, 0.01f);
    ...
    EXPECT_NEAR(code.speedup_target, 0.0f, 0.01f);
    ...
    EXPECT_NEAR(rzn.speedup_target, 0.0f, 0.01f);
    ...
    EXPECT_NEAR(agt.speedup_target, 0.0f, 0.01f);
    ...
    EXPECT_NEAR(tool.speedup_target, 0.0f, 0.01f);
    ...
    EXPECT_NEAR(batch.speedup_target, 0.0f, 0.01f);
}
...
static void test_mode_overrides(void) {
    ...
    EXPECT_NEAR(max_perf.speedup_target, 0.0f, 0.01f);
    ...
    EXPECT_NEAR(max_qual.speedup_target, 0.0f, 0.01f);
...
static void test_task_parsing_and_descriptions(void) {
    ...
    EXPECT_TRUE(strstr(qwn_autopilot_describe_mode(QWN_MODE_MAX_PERFORMANCE), "unavailable") != NULL);
    EXPECT_TRUE(strstr(qwn_autopilot_describe_mode(QWN_MODE_BALANCED), "unavailable") != NULL);
    EXPECT_TRUE(strstr(qwn_autopilot_describe_mode(QWN_MODE_MAX_QUALITY), "unavailable") != NULL);
}
...
static void test_stress_autopilot_matrix(void) {
    for (int mode = 0; mode < 3; mode++) {
        for (int task = 0; task < 6; task++) {
            ...
            EXPECT_NEAR(cfg.speedup_target, 0.0f, 0.01f);
