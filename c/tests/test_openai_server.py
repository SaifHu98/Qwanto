...
class DispatcherTest(unittest.TestCase):
    ...
    def test_close_wakes_pending_generation_and_is_idempotent(self):
        ...
        thread.join(timeout=2)
        self.assertFalse(thread.is_alive())
        self.assertEqual(errors, ["qwanto engine is shutting down"])
...
class HTTPTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.server.active_backend = NativeBackend("native", cls.server)
    ...
    def test_chat_completion(self):
        with self.request("/v1/chat/completions", {
            ...
            queue_wait = response.headers.get("x-qwanto-queue-wait-ms")
...
class SchedulerHTTPTest(unittest.TestCase):
    def setUp(self):
        ...
        self.server.active_backend = NativeBackend("native", self.server)
        self.thread = threading.Thread(target=self.server.serve_forever, daemon=True)
        self.thread.start()
