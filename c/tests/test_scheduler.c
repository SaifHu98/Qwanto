...
int main() {
    ...
    if (sched_route_expert(1, 4096, 4096, 0, 1) != TIER_CPU) return fail("Pinned expert S=1 should be CPU");
    if (sched_route_expert(64, 4096, 4096, 0, 1) != TIER_GPU) return fail("Pinned expert S=64 should be GPU");

    /* Unpinned expert (resident=0). CPU=1.0ms/row, GPU=3.5ms + 0.1ms/row */
    /* For S=1: CPU=1.0ms, GPU=3.6ms -> CPU */
    if (sched_route_expert(1, 4096, 4096, 0, 0) != TIER_CPU) return fail("Unpinned expert S=1 should be CPU");
    
    /* For S=8: CPU=8.0ms, GPU=3.5 + 0.8 = 4.3ms -> GPU */
    if (sched_route_expert(8, 4096, 4096, 0, 0) != TIER_GPU) return fail("Unpinned expert S=8 should be GPU");
