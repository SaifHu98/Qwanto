...
def build_plan(model, ram_gb=0, context=4096, gpu_indices=None, vram_gb=0,
    ...
    if ram_gb > 0:
        ...
        ram_budget = int(available_memory * 0.70)
        if available_memory <= 4 * GB or ram_budget < 4 * GB:
            ram_budget = int(min(ram_budget, 1.5 * GB))
            pc_reserve = int(0.2 * GB)
            act_reserve = int(0.2 * GB)
            mmap_enabled = True
        else:
    ...
    runtime_bytes = int(act_reserve + pc_reserve + 64 * typical + kv_bytes + kv_buffer)
    dense_resident_bytes = 0 if mmap_enabled else info["dense_bytes"]
    cache_bytes = max(0, ram_budget - dense_resident_bytes - runtime_bytes)
...
def environment_for_plan(plan, env=None, cuda_enabled=True):
    ...
    result = dict(env or {})
    result.setdefault("QWANTO_POLICY", plan["policy"]["name"])
    ...
    if ram['budget_bytes'] < 12 * GB:
        result.setdefault("QWANTO_MMAP", "1")
        result.setdefault("COLI_MMAP", "1")
    ...
    if not cuda_enabled or not devices or vram["budget_bytes"] <= 0:
        return result
    if result.get("QWANTO_CUDA", result.get("COLI_CUDA", "1")) == "0":
        return result

    result.setdefault("QWANTO_CUDA", "1")
    result.setdefault("COLI_CUDA", "1")
    if "QWANTO_GPU" not in result and "QWANTO_GPUS" not in result and "COLI_GPU" not in result and "COLI_GPUS" not in result:
        key = "QWANTO_GPU" if len(devices) == 1 else "QWANTO_GPUS"
        coli_key = "COLI_GPU" if len(devices) == 1 else "COLI_GPUS"
        result[key] = ",".join(map(str, devices))
        result[coli_key] = ",".join(map(str, devices))
