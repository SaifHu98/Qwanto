...
#include <fcntl.h>
  ...
  #define ST_HAS_UNISTD 0
  #define ST_HAS_DIRENT 0
#elif defined(__has_include)
  #if __has_include(<unistd.h>)
    #define ST_HAS_UNISTD 1
  #else
    #define ST_HAS_UNISTD 0
  #endif
  #if __has_include(<dirent.h>)
    #define ST_HAS_DIRENT 1
  #else
    #define ST_HAS_DIRENT 0
  #endif
#else
  #if defined(__unix__) || defined(__linux__) || defined(__APPLE__) || defined(__MINGW32__)
    #define ST_HAS_UNISTD 1
    #define ST_HAS_DIRENT 1
  #else
    #define ST_HAS_UNISTD 0
...
static void st_init(shards *S, const char *snap_dir) {
    ...
    for (int fi = 0; fi < nf; fi++) {
        ...
        for (int i = 0; i < root->len; i++) {
            ...
            /* offset declarat dal file: non-negativi, ordinati e dentro al
...
#else
static void st_init(shards *S, const char *snap_dir) {
    // Stub for Windows environments lacking dirent.h/unistd.h
    (void)S; (void)snap_dir;
}
#endif
...
static int64_t st_read_f32(shards *S, const char *name, float *out, int drop) {
    ...
    if (t->dtype == 2) {
        /* Direct zero-copy read for Float32 tensors */
        if (pread(t->fd, out, t->nbytes, t->off) != t->nbytes) { perror("pread data f32"); exit(1); }
    } else {
...
static void st_read_slice_f32(shards *S, const char *name, int64_t elem_off, int64_t n_elems, float *out, int drop) {
    ...
    if (t->dtype == 2) {
        /* Direct zero-copy slice pread for Float32 */
        if (pread(t->fd, out, nb, boff) != nb) { perror("pread slice f32"); exit(1); }
    } else {
        void *raw = malloc(nb);
        ...
        if (t->dtype == 0) { uint16_t *p = (uint16_t *)raw; for (int64_t i = 0; i < n_elems; i++) out[i] = bf16_to_f32(p[i]); }
        else { uint16_t *p = (uint16_t *)raw; for (int64_t i = 0; i < n_elems; i++) out[i] = f16_to_f32(p[i]); }
        free(raw);
    }
