...
__device__ static float qwn_half(const uint8_t *p) {
    __half_raw raw; raw.x=(unsigned short)(p[0]|((unsigned short)p[1]<<8));
    return __half2float(raw);
}

__global__ static void qwn_q4_matmul(float *y,const float *x,const uint8_t *w,
                                     int S,int I,int O,size_t row_bytes){
    int o=blockIdx.x,s=blockIdx.y;float sum=0.0f;
    const uint8_t *row=w+(size_t)o*row_bytes;const float *xr=x+(size_t)s*I;
    for(int i=threadIdx.x;i<I;i+=blockDim.x){
        int b=i>>5,j=i&31;const uint8_t *block=row+(size_t)b*18;
        uint8_t packed=block[2+(j>>1)];int q=((j&1)?packed>>4:packed&15)-8;
        sum+=xr[i]*(float)q*qwn_half(block);
    }
    __shared__ float partial[256];partial[threadIdx.x]=sum;__syncthreads();
    for(int n=blockDim.x>>1;n;n>>=1){if(threadIdx.x<n)partial[threadIdx.x]+=partial[threadIdx.x+n];__syncthreads();}
    if(!threadIdx.x)y[(size_t)s*O+o]=partial[0];
}
...
extern "C" int coli_cuda_qwn_matmul(ColiCudaTensor **tensor,float *y,const float *x,
                                      const void *blocks,int S,int I,int O,int device){
    DeviceContext *ctx=find_ctx(device);
    if(!tensor||!blocks||!x||!y||S<1||I<1||O<1||!select_ctx(ctx))return 0;
    size_t rb=(size_t)((I+31)/32)*18,wb=rb*(size_t)O;
    if(!*tensor){
        ColiCudaTensor *t=(ColiCudaTensor*)std::calloc(1,sizeof(*t));if(!t)return 0;
        t->fmt=5;t->I=I;t->O=O;t->device=device;t->weight_bytes=wb;
        if(!cuda_ok(cudaMalloc(&t->weights,wb),"qwn tensor allocation")||
           !cuda_ok(cudaMemcpy(t->weights,blocks,wb,cudaMemcpyHostToDevice),"qwn tensor upload")){
            coli_cuda_tensor_free(t);return 0;
        }
        t->tracked=1;ctx->tensor_count++;ctx->tensor_bytes+=wb;*tensor=t;
    }
    ColiCudaTensor *t=*tensor;if(t->fmt!=5||t->I!=I||t->O!=O||t->device!=device)return 0;
    size_t xb=(size_t)S*I*sizeof(float),yb=(size_t)S*O*sizeof(float);
    if(!reserve(&ctx->x,&ctx->x_cap,xb)||!reserve(&ctx->y,&ctx->y_cap,yb))return 0;
    if(!cuda_ok(cudaMemcpyAsync(ctx->x,x,xb,cudaMemcpyHostToDevice,ctx->stream),"qwn input upload"))return 0;
    qwn_q4_matmul<<<dim3((unsigned)O,(unsigned)S),256,0,ctx->stream>>>(ctx->y,ctx->x,
        (const uint8_t*)t->weights,S,I,O,rb);
    return cuda_ok(cudaGetLastError(),"qwn matmul launch")&&
           cuda_ok(cudaMemcpyAsync(y,ctx->y,yb,cudaMemcpyDeviceToHost,ctx->stream),"qwn output download")&&
           cuda_ok(cudaStreamSynchronize(ctx->stream),"qwn matmul sync");
}
