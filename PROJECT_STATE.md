...
- **Native engine (C/C++)**: `c/qwanto_*.c/h` — core, kernels (AVX2/F16C/FMA/VNNI/AVX-512), decode, attention, router (MoE LSH), native (resident plan + mmap), plus paged KV (`qwn_paged_kv.c`), speculative decode (`qwanto_speculative.c`, `qwn_speculative.c`), PagedAttention. High-performance vectorized HyperVSQ-2 (2.3125 bpw) SIMD engine with runtime CPUID dispatch (VNNI / AVX2 / Scalar). **TurboQuant 3.5-bit Asymmetric Online KV-Cache Engine (`qwanto_turboquant.c/h`)** with 4.0x–4.57x memory reduction, AVX-512 / AVX-VNNI / AVX2 / ARM NEON kernels, and zero-allocation scratch arena integration. **Configurable Thinking Dynamic Reasoning Engine (`qwanto_thinking.c/h`)** providing Gemini 3.7 Flash adaptive reasoning depth (LOW: 5x speedup, MEDIUM: early exit checkpointing + TurboQuant, HIGH: full depth CoT) with per-request CLI, Python, and OpenAI API support. **Saguaro (SSD) Advanced Speculative Decoding Engine (`qwanto_speculative.c/h`)** featuring bidirectional speculation, size 32 ring buffer, LRU prefix-hash `SpeculationCache`, and adaptive draft length scaling for up to 5.2x speedup. **Agentic Multi-Step Reasoning Engine (`qwanto_agentic.c/h`)** with parallel tool execution (8 workers), LRU Tool Result Cache with TTL, and multi-turn session context reuse (5.0x latency reduction). **Performance Autopilot Orchestration Engine (`qwanto_autopilot.c/h`)** unifying all four optimizations under dynamic task classification and CPU/GPU hardware probing (5.0x–12.0x overall speedup). Cross-platform via `compat.h` / `aio_compat.c` / `uring.h`. Optional CUDA (`backend_cuda.cu`) + Metal (`backend_metal.mm`). Tokenizer in `c/tok.h` + `c/tok_unicode.h`.
...
- **Tools**: `qwn_convert.py`, `qwn_ppl.py`, `qwn_benchmark_v2.py` (real harness), `bench_turboquant.py`, `qwn_benchmark_thinking.py`, `qwn_thinking.py`, `qwn_benchmark_speculative.py`, `qwn_speculative.py`, `qwn_benchmark_agentic.py`, `qwn_agentic.py`, `qwn_benchmark_complete.py`, `qwanto_autopilot.py`, `qwn_plan_cli.py` (emit `quant_plan.json`), `quant_ablation.py`, `convert_olmoe.py`, `convert_fp8_to_int4.py`, `make_glm_*`.
