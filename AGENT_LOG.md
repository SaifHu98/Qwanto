...
## 2026-08-27 — Qwanto Studio, Q8_0 conversion, and CUDA verification (Codex)

- Reworked Qwanto Code into the Qwanto Studio visual shell and exposed Q8_0 in
  the web converter and model settings.
- Added verified Q8_0 output for dense F32/F16/BF16 matrices and GGUF Q8_0/K-
  quant sources; documented the new writer while retaining fail-closed gaps.
- Restored robust Visual Studio discovery and SM_120 ABI build wiring.
- Validation: Python 254 passed/4 skipped; focused conversion/acceleration 29
  passed; native C 17/17; Web 61/61 plus production build; CUDA ABI v1
  synthetic test passed on device 0; CUDA DLLs rebuilt with CUDA 13.3/MSVC
  19.44. No claim of all-format support, paper-equivalent TurboQuant, or
  general performance uplift.
- Follow-up: added exact streamed GGUF source decoders for Q2_K, Q3_K, Q8_K,
  IQ2_XXS/XS/S, IQ3_XXS/S, and IQ4_XS/NL, with canonical compressed grid
  tables and direct differential checks against the llama.cpp Python reference
  (29/29 random blocks exact). Full Python validation is 257 passed/4 skipped
  with 10 conversion subtests. IQ1_S,
  native IQ/K dtypes, unsupported architectures, and paper-equivalent
...
2026-08-22 | user + Antigravity | Reviewed and pushed the local heterogeneous-acceleration commit (2a9bba4) which was on disk but had not yet been pushed to origin. The commit adds optimised CUDA GEMV (4 warps/block, register accumulation, persistent events) + SM_120 build target + GGUF Q8_0 vectorized dequant + native_smoke_test SNAP env fix + early stderr capture on engine exit + qwn_prefetch_batch (NVMe PrefetchVirtualMemory batch) + zero-alloc speculative buffers. Validated locally: web 61/61 tests + production build, python c/tests/ 253 passed 4 skipped, native 17/17 binaries, qwn_cuda.dll rebuild against the modified qwn_hypervsq2_cuda_abi.cu succeeds. Re-measured 4B HyperVSQ-2 on RTX 5070 Ti Laptop with the new qwnrun.exe SHA bf66b211f3ed4e755c9e1ba30655f9ccabf30910104601a373aed241b2e68f94 (binary mtime 2026-08-22T15:46:31Z):  CUDA 18.286 tok/s at 128 tok (gpu_matmul_count=10816, cpu_fallback_count=0), CPU 13.722 tok/s at 128 tok. CUDA is now 33.3% faster than CPU on this hardware (prior binary had CUDA 5.9% slower than CPU, so the relative ordering flipped across the binary boundary). The 122% CUDA gain vs the prior 8.22 tok/s run includes compiler/optimizer drift as well as the explicit kernel retune; isolating the kernel contribution alone would require an A/B binary of qwn_hypervsq2_cuda_abi.cu which has not been produced. | README.md; docs/performance-report.{md,json}; benchmarks/benchmark_matrix.json; benchmarks/evidence/windows/2026-08-22/4b_*_v2*.{txt,json}; PROJECT_STATE.md; AGENT_LOG.md; .github/workflows/{ci,release}.yml; c/build_cuda.bat; c/cuda/qwn_hypervsq2_cuda_abi.cu; c/model_acquisition.py; c/openai_server.py; c/qwanto_decode.c; c/qwanto_native.c; c/qwanto_native.h; c/qwn_speculative.{c,h}; c/qwnrun.c; c/tools/qwn_convert.py | Web 61/61, Python 253 passed 4 skipped, Native C 17/17, qwn_cuda.dll rebuilt clean from .cu source via build_cuda.bat, 4B CUDA remeasured 18.286 tok/s at 128 tok vs CPU 13.722 tok/s on RTX 5070 Ti Laptop. | No benchmark SKU claim without executable+model hash pinning. The 2a9bba4 commit only documents local measurements; no claim about other hardware or other model classes. The 'Phase 1' published-speedup text is not added to README because the CUDA gain depends on binary rebuild and is not uniformly reproducible across the random CI runner fleet.

2026-08-27 | Codex | Added the native CPU Qwen3.8 main-path integration: exact GGUF source mapping, Q2_K/Q3_K/Q8_K native QWN scalar payloads/kernels, Qwen3.8 Gated DeltaNet recurrent state with causal convolution and attention-gate handling, and compatibility preservation for legacy FFN-only projected fixtures. Converted the local Qwen3.8-27B IQ2_M source to an ignored Q4_0 QWN artifact and executed one real native CPU token successfully. | c/qwanto_decode.c; c/qwanto_decode.h; c/qwanto_kernels.c; c/qwanto_native.c; c/qwanto_native.h; c/tools/qwn_convert.py; c/tools/qwen38_qualification.py; c/tests/test_qwn_conversion_safety.py; c/tests/test_qwn_kernels.c; c/tests/test_real_models.py; c/tests/test_qwen38_qualification.py; docs/qwen38-27b-qualification.md; docs/qwn-supported-quantizations.md; docs/converter-capability-matrix.md; docs/dtype-support-roadmap.md; docs/acceleration-roadmap.md; PROJECT_STATE.md | `python -m pytest c/tests/ -q`: 258 passed, 4 skipped, 10 subtests passed; Qwen3.8 local native run: `status=ok`, 1 token, CPU AVX2/FMA/F16C, 24.610000 seconds. `make -C c test-c` was not run successfully because Make is unavailable in this PowerShell environment. No benchmark evidence or README performance row changed. | CPU main-path integration only; native IQ, MTP execution, MoE dispatch, hybrid CUDA, model-quality oracle, and paper-equivalent TurboQuant remain gated and unclaimed.

2026-08-27 | Codex | Added official Qwen3.8 model presets and direct Qwanto Code download actions: Flash-Next UD-Q4_K_XL as a four-shard ~111 GB bundle and Qwen3.8-27B Q4_K_M as the ~19 GB lightweight fallback. The downloader resumes and atomically publishes each shard, records bundle progress in `.qwanto-bundle.json`, and keeps missing-checksum artifacts explicitly unverified/external-source-only. | c/model_acquisition.py; c/openai_server.py; c/tests/test_model_acquisition.py; c/tests/test_gateway_integration.py; web/src/lib/api.ts; web/src/components/DesktopSettingsView.tsx; web/src/index.css; docs/model-acquisition-design.md; docs/converter-capability-matrix.json; c/tests/test_acceleration_contracts.py; PROJECT_STATE.md | Official preset metadata and URL construction tests passed; loopback two-shard download, SHA-256, atomic publication, and resume manifest tests passed; gateway integration 11 passed; Web 61 passed and `npm run build` clean; full Python suite after aligning the stale architecture assertion: 260 passed, 4 skipped, 10 subtests. The incorrect `npm test -- --runInBand` invocation failed because Vitest does not support that Jest flag; plain `npm test` passed. No model download, native runtime claim, benchmark evidence, README performance row, tag, or release changed. | Direct acquisition/UI support is complete for official source artifacts only. Flash-Next native IQ/MTP/MoE/CUDA DeltaNet, QWN activation, quality oracle, benchmark, and paper-equivalent TurboQuant remain unimplemented and fail closed.
2026-08-27 | Codex | Added native QWN IQ2/IQ3/IQ4 tensor-row runtime for the supported GGML block layouts, byte-preserving `--quant none` conversion, canonical grid data, RESET protocol support, reproducible logit-oracle and native benchmark tools, and bounded redacted desktop error logging. | c/ggml_iq_grids.h; c/qwanto_native.*; c/qwanto_kernels.c; c/tools/qwn_convert.py; c/tools/qwn_quality_oracle.py; c/tools/qwn_benchmark.py; c/qwnrun.c; c/tests/test_qwn_conversion_safety.py; c/tests/test_qwn_quality_tools.py; c/tests/test_acceleration_contracts.py; desktop/src-tauri/src/{diagnostics,lib}.rs; web/src/App.tsx; docs/ | Python `264 passed, 4 skipped, 17 subtests passed`; C builder `17/17` passed; IQ differential tests passed for IQ2_XXS/XS/S, IQ3_XXS/S, IQ4_NL/XS; Web `61/61` and production build passed; full Clang qwnrun build and `--build-info --json` passed; Cargo check is blocked by missing packaged `desktop/src-tauri/resources/*`, not Rust source diagnostics. | Native IQ tensor/runtime-row support is measured, not full Flash-Next support. MTP, sparse MoE/QSA/ngram/gated residual, CUDA hybrid DeltaNet, paper TurboQuant, and official Flash-Next end-to-end quality/performance remain fail-closed and unclaimed; no benchmark evidence or model weights changed.
2026-08-27 | Codex | Prepared release candidate `v0.1.0-beta.7` from the validated working tree. | commit `b4bc46b`; local annotated tag `v0.1.0-beta.7` | Python `264 passed, 4 skipped, 17 subtests`; C `17/17`; Web `61/61` plus production build; release workflow policy and `git diff --check` passed. | GitHub rejected both `main` and tag pushes with HTTP 403 Permission denied; `gh` CLI is unavailable, so no remote publication or GitHub release exists. Local tag remains intact.
2026-08-27 | Codex | Published `main` and `v0.1.0-beta.7` after the PAT was corrected; diagnosed and repaired Windows release-build macro quoting so Clang receives a valid `QWN_BUILD_OPT_FLAGS` definition. | .github/workflows/ci.yml; .github/workflows/release.yml; c/Makefile; c/qwnrun.c; PROJECT_STATE.md | PAT API identity `SaifHu98` with repository push permission; remote refs verified at `5445240`; local Windows-style macro syntax check and release-policy check passed. Workflow `33113738318` failed only at Windows `Build qwnrun on Windows` with unquoted macro tokens; Ubuntu/macOS were still in progress at diagnosis. | The already-published tag must be force-updated to the fixed commit to rerun packaging; no Release assets are claimed until the new workflow completes.
2026-08-27 | Codex | Force-updated `v0.1.0-beta.7` to the Windows quoting fix and confirmed the replacement packaging workflow is running on all three platforms. | .github/workflows/ci.yml; .github/workflows/release.yml; c/Makefile; c/qwnrun.c; PROJECT_STATE.md | Remote `main` and tag resolve to `6b973e2`; workflow `33114350361` is `in_progress` for Windows, macOS, and Ubuntu. | Release assets remain unclaimed until the workflow reaches a final successful conclusion; local `c/qwn_speculative.h` user change remains uncommitted and outside the release commit.
2026-08-28 | Codex | Added an explicit-consent GitHub Issue gateway action and UI, plus a native pointer-resolved separated-expert MoE top-k dispatcher with converter metadata and a synthetic selected-expert fixture. | c/openai_server.py; c/qwanto_decode.*; c/tools/qwn_convert.py; c/tests/test_openai_server.py; c/tests/test_qwn_decoder.py; web/src/lib/api.ts; web/src/__tests__/api_integration.test.ts; web/src/components/DesktopSettingsView.tsx; PROJECT_STATE.md | Gateway tests `40 passed`; web API tests `14 passed`; isolated native decoder compilation passed. Full native decoder suite is blocked before execution by the pre-existing local `c/qwn_speculative.c`/modified `c/qwn_speculative.h` field mismatch. | No GitHub issue was created, no token was stored, no benchmark evidence changed; MTP, Flash-Next-specific packed experts/QSA/GR, hybrid CUDA, paper TurboQuant, and Flash-Next quality/performance remain unclaimed.
2026-08-28 | Codex | Restored the speculative-context fields required by the native implementation, added a CPU TurboQuant paper-reference KV mode (random rotation, scalar Lloyd-Max codebook, normalized Gaussian QJL residual), and added native Qwen NextN/MTP head execution with an independent FP16 KV state and `MTP_FORWARD` protocol operation. | c/qwn_speculative.h; c/qwanto_turboquant.*; c/qwn_runtime_config.*; c/qwanto_decode.*; c/qwnrun.c; c/tests/test_turboquant_paper.c; c/tests/test_qwn_decoder.py; c/tools/build_and_run_c_tests.py; c/Makefile; docs/turboquant-kv-cache.md; docs/qwen38-27b-qualification.md; PROJECT_STATE.md | Python `268 passed, 4 skipped, 17 subtests`; native C suite `18/18` passed; web `62/62` plus production build passed; TurboQuant multi-seed QJL estimator and synthetic MTP base-plus-head fixtures passed. | `turboquant-paper` is an O(d²) CPU correctness reference, not a performance mode; MTP is not yet a speculative accept/reject path; Flash-Next quality, packed MoE/QSA/GR, and hybrid CUDA remain unmeasured/unclaimed.
2026-08-28 | Codex | Completed the native MTP speculative transaction and wired it to qwnrun and the typed gateway runtime configuration. The implementation checkpoints MTP state, restores it on rejection, preserves target probabilities before MTP reuses the logits buffer, rejects external draft models and unsupported top-p truncation, and exposes factual request counters. | c/qwanto_decode.*; c/qwn_speculative.*; c/qwn_runtime_config.c; c/qwnrun.c; c/openai_server.py; c/tests/test_qwn_decoder.py; PROJECT_STATE.md; docs/qwen38-27b-qualification.md | Python `269 passed, 4 skipped, 17 subtests`; native C suite `18/18` passed. The native protocol test forces an MTP draft rejection, confirms greedy output is byte-identical to ordinary target generation, and records a non-zero rejection counter. | This is synthetic transaction-correctness evidence only: the target verifier remains sequential, so no throughput, real-Qwen3.8 quality, or Flash-Next performance claim is made.
2026-08-28 | Codex | Fixed the GGUF NextN/MTP tensor-name mapping, which had truncated the first suffix character and made the real Qwen3.8 artifact fail native shape validation. Reconverted a new ignored local QWN artifact and ran a real one-token MTP speculative comparison. | c/tools/qwn_convert.py; c/tests/test_qwn_conversion_safety.py; models/Qwen3.8-27B-Q4_0-mtp-fixed.qwn (ignored); PROJECT_STATE.md; docs/qwen38-27b-qualification.md | Structural inspection confirms `eh_proj`, `enorm`, `hnorm`, and `shared_head_norm`; prompt `Hi`, greedy sampling, and draft length 1 completed `NATIVE_MTP_COMPLETED` with 1 proposal/0 accepts/1 rejection and emitted token `2`. Target-only greedy emitted the same token. | Separate cold CPU runs took 67.016 s speculative and 51.704 s baseline, so this is correctness evidence only and does not support a speed claim.
2026-08-28 | Codex | Audited the available local model library and added a reproducible three-artifact CPU diagnostic comparison for Q4_0, BF16 baseline, and HyperVSQ-2, including conversion sizes, runtime telemetry, and explicit exclusions for duplicate or unsupported files. | README.md; PROJECT_STATE.md; AGENT_LOG.md | Current-source temporary qwnrun: 1.5B Q4_0 `0.924933 tok/s` decode, 4B BF16 `0.299913 tok/s`, 4B HyperVSQ-2 `0.430528 tok/s`; conversion records show 4B HyperVSQ-2 at `1,266,202,104` B versus `8,655,205,784` B baseline. Full model-library audit found no second valid HyperVSQ-2 model. | Measurements are cold local diagnostics from a non-OpenMP/non-release-ISA binary; no benchmark evidence or universal speed claim was promoted.
2026-08-28 | Codex | Published the new Qwanto Native agent release candidate with the native MTP transaction, corrected NextN conversion mapping, model audit table, and factual test/diagnostic record. | commit `6458031`; annotated tag `v0.1.0-beta.8`; `main`; README.md; PROJECT_STATE.md; AGENT_LOG.md | Remote refs verified: `main` and the tag initially resolve to `6458031`; CI run `3318329` and Package run `3318330` started for the published commit. | Release assets remain unclaimed until GitHub CI and packaging finish; model weights and build artifacts are not included.
